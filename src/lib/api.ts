...
export const calculateLotSizes = async (symbol: string, entryPrice: number, stopLoss: number, riskPercentage: number = 1, leverage: number = 100) => {
  const response = await fetch('https://cgmzxonyaiwtcyxmmhsi.supabase.co/functions/v1/lot-size-calculator', {
    method: 'POST',
    headers: {
      'Content-Type': 'application/json',
      'Authorization': `Bearer eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6ImNnbXp4b255YWl3dGN5eG1taHNpIiwicm9sZSI6ImFub24iLCJpYXQiOjE3NTE5NzA2MzYsImV4cCI6MjA2NzU0NjYzNn0.jv5vMtuiLSmJijipBAVlTwjzXp123IDBA9kslT9kQEM`
    },
    body: JSON.stringify({ symbol, entryPrice, stopLoss, riskPercentage, leverage })
  });
